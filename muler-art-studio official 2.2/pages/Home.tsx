...
const REVIEWS_BOOKSY = [
  ...
  "«Muy buen corte👌 Mi nueva referencia en Murcia Center.» — Marco",
];
...
export default function Home() {
  ...
  return (
    <div>
      <SEO 
        title={isGateway ? "Barbería Murcia Centro | MULER ART STUDIO | Barbero Murcia" : "Mejor Barbería en Murcia | MULER ART STUDIO | Estilo & Precisión"}
        description="Experiencia de barbería premium en el centro de Murcia. Especialistas en corte técnico, degradados (fade) y cuidado masculino de alto nivel en San Basilio."
        schema={faqSchema}
      />
      ...
      {/* Teaser Services */}
      <section className="py-20 md:py-32 px-4 md:px-16 bg-black relative">
        <div className="max-w-[1600px] mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8">
            {SERVICES.slice(0, 4).map((s, idx) => (
              <Link to={`/${s.slug}`} key={s.id} className="group relative aspect-[4/5] md:aspect-[3/4] overflow-hidden border border-white/5 p-6 md:p-10 lg:p-14 flex flex-col justify-end bg-zinc-950 rounded-xl md:rounded-2xl transition-all duration-700 hover:border-white/20">
                <div className="absolute inset-0 z-0 overflow-hidden">
                  {isVideo(s.image) ? (
                    ...
                    <img 
                      ...
                      alt={`${s.title} en Murcia - Barbería de Autor Muler Art Studio`} 
                      width="600"
                      height="800"
                      decoding="async"
                      className="w-full h-full object-cover img-premium group-hover:scale-110 transition-all duration-1000 ease-out will-change-transform transform-gpu"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-24">
            <div className="lg:col-span-5">
              ...
              <p className="text-zinc-100 font-light text-xl leading-relaxed max-w-md tracking-wide">
                No solo cortamos el cabello; diseñamos una identidad. Como barbería en Murcia, cada ritual está concebido para ofrecer precisión técnica cerca de puntos clave como el barrio de <strong>San Basilio</strong> y la sede de <strong>La Opinión</strong>.
              </p>
            </div>
              ...
              {IMAGES.portfolio.slice(0, 4).map((img, i) => (
                <div key={i} className="aspect-[4/5] overflow-hidden rounded-xl neon-border group">
                  <img 
                    ...
                    alt={`Corte degradado fade realizado en barbería Murcia - Trabajo de barbero ${i}`} 
              ...
              >
                <div className="aspect-[16/10] overflow-hidden rounded-2xl relative">
                  <img 
                    ...
                    alt={`Artículo de estilo: ${post.title} - Muler Art Studio Murcia`} 
      ...
      {/* Location Section */}
      <section className="py-20 md:py-40 bg-black px-4 md:px-8 relative overflow-hidden">
        <div className="max-w-[1600px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-32 items-stretch">
          <div className="relative flex flex-col justify-center bg-zinc-950/50 p-8 md:p-20 rounded-3xl border border-white/5 overflow-hidden">
             <div className="relative z-10">
                <div className="flex items-center gap-4 mb-6 md:mb-10">
                   <div className="w-12 h-[1px] bg-white/20"></div>
                   <span className="text-zinc-200 uppercase tracking-[0.5em] text-[10px] font-black uppercase">Localización San Basilio</span>
                </div>
                
                <div className="flex flex-col gap-6 mb-10 md:mb-16">
                   <div className="flex items-start gap-3 md:gap-4">
                      <MapPin className="text-white mt-1 md:mt-2 shrink-0" size={24} md:size={32} strokeWidth={1}/>
                      <h3 className="text-3xl md:text-6xl font-serif italic text-white leading-tight">
                        {CONTACT.address}
                      </h3>
                   </div>
                   <p className="text-zinc-100 text-base md:text-lg font-light tracking-widest md:pl-12">
                     Barrio de San Basilio • Murcia Capital (Al lado de La Opinión)
                   </p>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 md:gap-12 md:pl-12">
                ...
                <div className="mt-12 md:mt-20 md:pl-12 flex flex-col sm:flex-row gap-4 md:gap-8">
                   <a href={CONTACT.booksyUrl} className="booksy-widget-button text-center text-[10px] uppercase tracking-widest font-black text-black bg-white px-8 py-4 md:px-12 md:py-5 rounded-full hover:bg-zinc-200 transition-all uppercase">
                      Pedir Cita Barbería
                   </a>
                   <Link to="/contacto" className="text-[10px] uppercase tracking-widest font-black text-white border border-white/10 px-8 py-4 md:px-12 md:py-5 rounded-full hover:bg-white/5 transition-all uppercase text-center">
                      Mapa San Basilio
                   </Link>
                </div>
             </div>
          ...
          <div className="h-[400px] md:h-[500px] lg:h-auto min-h-[400px] md:min-h-[600px] rounded-3xl overflow-hidden grayscale invert brightness-[0.6] contrast-[1.3] opacity-50 hover:opacity-100 hover:brightness-100 hover:grayscale-0 hover:invert-0 transition-all duration-[1.5s] relative z-20 border border-white/10">
            <iframe 
              ...
              title="Barbería en Murcia Barrio San Basilio MULER ART STUDIO"
            ></iframe>
          </div>
        </div>
      </section>
