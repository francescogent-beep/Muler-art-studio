    ...
    <>
          ...
          <nav className="hidden md:flex gap-12 items-center">
            {links.map(l => (
              <Link 
                key={l.path} 
                to={l.path} 
                className={`text-[10px] uppercase tracking-[0.4em] font-medium transition-all hover:text-white ${location.pathname === l.path ? 'text-white' : 'text-zinc-200'}`}
              >
                {l.name}
              </Link>
            ))}
            <Link to="/reservar" className="ml-4 border border-white/20 px-8 py-2 text-[10px] uppercase tracking-[0.3em] hover:bg-white hover:text-black transition-all duration-500 text-white rounded-full">
              RESERVAR
            </Link>
          </nav>
      ...
      >
        <div className="flex flex-col items-center gap-6 mb-12 px-6 text-center">
          {links.map((l, i) => (
            <Link 
              key={l.path} 
              to={l.path} 
              className={`text-4xl md:text-5xl font-serif italic transition-all duration-500 hover:text-zinc-100 ${location.pathname === l.path ? 'text-white' : 'text-zinc-300'}`}
              style={{ 
        ...
        >
          <Link 
            to="/reservar" 
            className="flex items-center justify-center gap-3 border border-white/20 py-5 text-[10px] uppercase tracking-[0.3em] font-bold text-white rounded-full hover:bg-white hover:text-black transition-all"
          >
            RESERVAR CITA
          </Link>
          <a 
            href={`tel:${CONTACT.phone}`} 
            className="flex items-center justify-center gap-3 border border-white/10 py-5 text-[10px] uppercase tracking-[0.3em] font-bold text-zinc-100 rounded-full"
          >
    ...
    <footer className="bg-black pt-32 pb-16 border-t border-white/10">
      <div className="max-w-[1800px] mx-auto px-8 md:px-16 grid grid-cols-1 md:grid-cols-12 gap-20">
        <div className="md:col-span-4">
          <h2 className="font-logo text-5xl mb-10 italic text-white leading-tight">Muler Art <br/><span className="not-italic text-zinc-100 font-light">Studio.</span></h2>
          <p className="text-zinc-200 max-w-sm text-sm leading-relaxed font-light mb-12 italic">
          ...
          <div className="flex gap-10">
            <a href="https://instagram.com/mulerartstudio" target="_blank" rel="noopener noreferrer" className="text-zinc-100 hover:text-white transition-colors flex items-center gap-2 text-[10px] uppercase tracking-widest font-bold">
              <Instagram size={16} strokeWidth={1}/> INSTAGRAM
            </a>
            <a href={`https://wa.me/${CONTACT.whatsapp}`} target="_blank" rel="noopener noreferrer" className="text-zinc-100 hover:text-white transition-colors flex items-center gap-2 text-[10px] uppercase tracking-widest font-bold">
          ...
          <FooterSection title="Navegación" defaultOpen={true}>
            ...
            <div className="flex flex-col gap-2 mt-2">
              ...
              <div className="flex flex-col gap-2 pl-4 border-l border-white/10 py-1">
                {SERVICES.map(s => (
                  <Link key={s.id} to={`/${s.slug}`} className="text-[10px] text-zinc-200 hover:text-white transition-colors uppercase tracking-widest">
                    {s.title}
                  </Link>
                ))}
              </div>
            ...
            <div className="flex flex-col gap-2 mt-6">
              <span className="text-[9px] uppercase tracking-[0.3em] text-zinc-300 font-bold uppercase">Información Legal</span>
              <div className="flex flex-col gap-2 pl-4 border-l border-white/10 py-1">
                <Link to="/legal/aviso-legal" className="text-[10px] text-zinc-200 hover:text-white transition-colors">Aviso Legal</Link>
                <Link to="/legal/privacidad" className="text-[10px] text-zinc-200 hover:text-white transition-colors">Privacidad</Link>
                <Link to="/legal/cookies" className="text-[10px] text-zinc-200 hover:text-white transition-colors">Cookies</Link>
              </div>
            </div>
          </FooterSection>
        </div>
          ...
          <div>
            ...
            <div className="flex flex-col gap-8 text-xs text-zinc-100 font-light leading-relaxed">
              <div className="flex items-start gap-4">
                <MapPin size={18} className="mt-1 text-zinc-100 shrink-0" strokeWidth={1.5}/>
                <p>
              ...
              <div className="flex items-center gap-4">
                <Phone size={18} className="text-zinc-100 shrink-0" strokeWidth={1.5}/>
                <p className="text-white font-medium">{CONTACT.phone}</p>
              </div>
              
              <div className="pt-8 border-t border-white/10">
                <h4 className="text-[9px] uppercase tracking-[0.4em] text-zinc-300 font-bold mb-4 uppercase">Horario de Apertura</h4>
                <div className="space-y-2">
                  {CONTACT.schedule.map((s, i) => (
                    <div key={i} className="flex justify-between text-[11px]">
                      <span className="text-zinc-200 font-medium uppercase">{s.day}</span>
      ...
      <div className="max-w-[1800px] mx-auto px-8 md:px-16 mt-24 pt-12 border-t border-white/10 flex flex-col items-center gap-8 text-[9px] text-zinc-300 uppercase tracking-[0.5em] font-bold">
        <div className="flex flex-col md:flex-row items-center gap-8 md:gap-12">
          <div className="flex gap-10">
            <span className="text-zinc-100 uppercase">Diseño & Técnica de Autor</span>
            <span className="text-zinc-100 uppercase">Hecho en Murcia</span>
          </div>
          <a 
            href="https://fgdigitalsystems.com" 
            target="_blank" 
            rel="noopener noreferrer" 
            className="text-zinc-100 hover:text-white transition-colors duration-500 md:border-l md:border-white/10 md:pl-12"
          >
            disegno web by <span className="text-zinc-100">FGDIGITALSYSTEMS</span>
          </a>
        </div>
        <span className="text-zinc-200 pt-4 border-t border-white/5 w-full text-center uppercase tracking-widest">
...
export default function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col selection:bg-white selection:text-black">
      ...
      <Link 
        to="/reservar"
        className="fixed bottom-6 left-6 right-6 md:left-auto md:right-10 md:bottom-10 md:w-auto z-[90] glass-premium px-8 py-5 md:py-4 flex items-center justify-center md:justify-start gap-4 group hover:bg-white hover:text-black transition-all duration-700 text-white rounded-full shadow-2xl"
      >
